...
jieba.load_userdict("dic/mydic2.txt")
...
train_set1 = []
guosheng = {}.fromkeys([line.rstrip() for line in open('guosheng.txt')])

stopwords = {}.fromkeys([line.rstrip() for line in open('mystopword.txt')])   #fromkeys该方法用于重建一个字典
    ...
    for line in reader:
        ...
        if str15 in guosheng:
            ...
            for seg in segs:
                ...
                #for w in words:
                    #if len(list(seg.decode("utf-8")))==1:
                        #break
                    #if w.flag.encode("utf-8") not in cipinstop:
                        #break
                    #if seg in stopwords:
                        #break
                    #final.append(seg)
...
dic.save('tem/quanguo3/dict_quanguo.dict')    #存字典在本地
...
corpora.MmCorpus.serialize('tem/quanguo3/corpus.mm', corpus)     #c存入硬盘以备后需，相似度接口会用到
...
tfidf.save('tem/quanguo3/tfidf_quanguo.tfidf_model')
...
lda = models.LdaModel(corpus_tfidf,id2word = dic,alpha='auto',eta='auto',num_topics = 25,iterations=1000)   #LDA模型  alpha='auto', eta='auto',
lda.save('tem/quanguo3/mylda_quanguo.pkl')
...
for j in range(0, 24):
    print(lda.print_topic(j,topn=15))
